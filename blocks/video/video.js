...
function loadVideoEmbed(block, link, autoplay, background, poster) {
  ...
  if (source === 'youtube') {
    ...
    const videoEl = getVideoElement(link, autoplay, background, poster);
    ...
    videoEl.addEventListener('play', () => {
      videoEl.setAttribute('data-video-state', 'playing');
    });
    videoEl.addEventListener('pause', () => {
      videoEl.setAttribute('data-video-state', 'paused');
    });
    videoEl.addEventListener('ended', () => {
      videoEl.setAttribute('data-video-state', 'ended');
    });
  }
}
...
export default async function decorate(block) {
  ...
  const link = block.querySelector('a[title="src"]')?.href || '';
  const poster = block.querySelector('a[title="poster"]')?.href || '';
  ...
  if (!placeholder || autoplay) {
    const observer = new IntersectionObserver((entries) => {
      if (entries.some((e) => e.isIntersecting)) {
        ...
        loadVideoEmbed(block, link, playOnLoad, autoplay, poster);
      }
    });
