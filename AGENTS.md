...
### Project Facts (verified 2026-07-23)
- **Repo:** `AdobeDrago/statefarm-b2b-eds` — branch `main`
- **Project type:** `doc` (DA / Document Authoring backed; see `.migration/project.json`)
- **Preview org/site:** `adobedrago` / `statefarm-b2b-eds`
- **Preview:** `https://main--statefarm-b2b-eds--adobedrago.aem.page/` · **Live:** `…aem.live/`
- **Blocks present:** cards, cards-service, columns, columns-auth, columns-promo, footer, fragment, header, hero, widget
- **Dev server:** `http://localhost:3000` (verified 200)

## Working with Claude — Skill Routing

Claude Code has AEM Edge Delivery skills installed. **Invoke the matching skill before starting work** — do not freelance a task a skill covers.

| Task | Skill |
|---|---|
| New feature / block / "let's build X" | `superpowers:brainstorming` → `aem-edge-delivery-services:content-driven-development` |
| Implement/modify block, scripts.js, CSS | `aem-edge-delivery-services:building-blocks` |
| Design the authoring content model for a block | `aem-edge-delivery-services:content-modeling` |
| Import / migrate an existing page | `aem-edge-delivery-services:page-import` |
| Preserve original design, make authorable | `aem-edge-delivery-services:snowflake` |
| Test a change before PR | `aem-edge-delivery-services:testing-blocks` |
| Self-review before PR | `aem-edge-delivery-services:code-review` |
| Any bug / test failure | `superpowers:systematic-debugging` |
| Find where a block is used | `aem-edge-delivery-services:find-test-content` |
| Search aem.live docs | `aem-edge-delivery-services:docs-search` |
| Admin ops (preview/publish/cache/users) | `aem-project-management:ops` |
| DA content push/preview | `aem-edge-delivery-services:da-content` + `aem-edge-delivery-services:da-auth` |
| Accessibility / metadata / audit at scale | `aem-eds-content-ops:*` |

**MCP servers available (via claude.ai):** AEM Content, AEM DA, AEM Odin, Adobe Express, Analytics. Auth is per-server on first use.

**DA auth note:** cached DA token at `~/.aem/da-token.json` was **expired** as of 2026-07-23. Any DA push/preview needs a refresh first — run the `aem-edge-delivery-services:da-auth` skill (or `da-auth-helper token`).
